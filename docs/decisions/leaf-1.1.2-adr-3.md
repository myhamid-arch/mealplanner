...
- `registry` holds the 32 v1 ops of AGT-6, the 9 BLD-8 R-10 ops and `portion_bias.set` (R-24): 42 public ops. `ChangeOp` is the discriminated union `{ kind, payload }`; `ChangeOpSchema` is the Zod union used by UI forms, proposals, insights and the agent.
