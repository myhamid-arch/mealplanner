...
PLN-11 budgets a day at ≤ 5 s and a week at ≤ 30 s on the reference fixture. G4 plans each F1 day, and the F1 week, in a fresh process, with no warm solve memo. The days run one after another; the three week plans run side by side. The HiGHS runtime is loaded first, and its load is not counted. G4 then measures `planDays` wall-clock and main-thread CPU time (`process.threadCpuUsage`). gate-check may run gates concurrently, so the budget is asserted on main-thread CPU time. The planner is single-threaded, so that CPU time is what the plan takes on an unloaded core. Wall-clock is printed beside it. Whole-process CPU (`process.cpuUsage`) is not used: it also counts V8's background WebAssembly compilation and GC threads, measured at about 2 s above wall-clock for an F1 day.
