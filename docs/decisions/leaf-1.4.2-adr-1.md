...
- Semantic tables (CP1 amendment 10): `macroColor` (P/C/F/kcal → fill, tint, text), `fitColor` (`in_tolerance` green, `flexible_miss` amber, `infeasible` red, `untargeted` grey, each with a label and an icon), `toneColor` for chips, `avatarPalette` + `avatarColor(key)`. `MacroRing`'s `fit` prop and every fit badge read `fitColor`; 1.4.4 imports these tables instead of re-declaring colours.
