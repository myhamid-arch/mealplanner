...
- Semantic tables (CP1 amendment 10): `macroColor` (P/C/F/kcal → fill, tint, text), `fitColor` (`in_tolerance` green, `flexible_miss` amber, `infeasible` red, `untargeted` grey, each with a label and an icon), `toneColor` for chips, and the avatar palette keyed by the names stored in `member.color` (`AVATAR_COLORS`: sea, aubergine, pomegranate, saffron, basil, tomato, olive, flour — the eight names the F1–F3 fixtures of leaf 1.1.2 store; `avatarPalette[name]` is a fill and the initial's colour, AA as text: olive is the mockups' #6E6E1F with a white initial (5.37:1), flour carries an ink initial (12.86:1)). `Avatar` takes `color` (the stored name), which wins; `resolveAvatarColor(stored, memberId)` falls back to a hash of the member id only when no colour is stored (CP3 finding 1). The family leaf offers `AVATAR_COLORS` as the choices. `MacroRing`'s `fit` prop and every fit badge read `fitColor`; 1.4.4 imports these tables instead of re-declaring colours.
