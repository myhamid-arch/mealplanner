...
Each question states the conservative reading this leaf builds on. None blocks a gate. Rulings: BLD-8 R-12 to R-14 (CP1 APPROVED).
...
Ruling: accepted (R-12). 1.1.2 adds `variant_ingredient.cooking_liquid` and `yield_override`. Absorbed stock counts its nutrients.
...
Ruling: accepted (R-12). The `coating_*` columns are removed from 02 §3.
...
Ruling: amended (R-13). Null propagates, except that `solubleFibre` counts as 0 when `fibre` is 0 and `sugar` counts as 0 when `carbs` is 0; the same applies in `plateNutrients`. Built that way, with golden cases for both exceptions and for a genuinely unknown value.
