...
## Decision
- **`pg` 8.23.0, parameterised SQL.** The store and the relational source take a structural `Queryable` (`{ query(text, values) }`), satisfied by `pg.Pool`, `pg.PoolClient` and `pg.Client`. Writes that must be atomic (one sync request, one rebuild) run inside `BEGIN … COMMIT` on one checked-out client (`pool.connect()`), rolled back on any error.
