...
| G2 | Each entry passes if either check is within 12 % (R-22):
- the generic check, `|kcal − (4P + 4C + 9F + 2·fibre)| / kcal ≤ 0.12`;
- the check with its own `meta.atwater_factors` applied to `C + fibre`.

An entry that passes carries no NUT-4 reason. Every entry that fails is `low` with a NUT-4 reason. Recorded factors are numeric and cite a source. `carbs_basis` is `available` (R-20), and a clamped entry is `low`. Counts are printed. | One entry's kcal scaled by 1.5; a failing entry's reason removed; a clamped entry left at high confidence; a factor entry with its factors removed; a factor entry with wrong factors; the snapshot regenerated by difference. |
