...
Defect codes: `unknown_ingredient`, `unknown_method`, `unknown_cuisine`, `unknown_slot`, `bad_serving_bounds`, `bad_default_variant`, `bad_new_ingredient`, `excluded_ingredient`, `excluded_category`, `excluded_dietary_flag`, `unverifiable_new_ingredient`, `variant_drift`, `atwater_variant`, `atwater_new_ingredient`, `nutrition_error`, `duplicate_name`, `duplicate_ingredients`, `infeasible`.
...
- Variant energy check (NUT-4 with R-22, as R-30 specifies for 1.2.4): the variant passes when the generic check (`atwaterCheck` on per-100 g cooked) is within 12 %, or when the variant's kcal is within 12 % of Σ over its ingredient contributions of the predicted energy with each ingredient's own `atwater_factors` (protein, fat, and carbohydrate on `carbs + fibre`, no fibre term) where recorded and 4/4/9/2 otherwise. Contributions are the same quantities the engine sums (fat retention applied to each ingredient's own fat; absorbed fat by the absorbed share). Implemented in this leaf until 1.2.4's core export exists (SPEC-Q-5).
