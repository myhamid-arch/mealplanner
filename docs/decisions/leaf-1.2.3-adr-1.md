...
Status: accepted at CP1 (R-34), with the build amendments listed at the end
...
2. **Kcal re-targeting pass (R-28).** Once the week's dishes are fixed (after the day beams and the improvement passes), each targeted member's plates are re-solved in time order, day by day. For the member's i-th attended slot, with `D` = the kcal deviation of the plates before it and `Bᵢ` = the sum of the resolver's kcal bands for slots 1…i:
...
## Build amendments (CP2)

These were measured during the build. None changes a result the CP1 design defines.

1. **Memo across runs (§1).** `solvePlate` results are memoised by input across plan runs, not only within one: per dish object (a `WeakMap`, so dishes that are no longer referenced release their entries), then by the member context, the target, the adjuster objects offered and the variant limits. Identity keys the objects, so an edited dish or adjuster (a new object) never reuses an old solution. Why: G3 plans the same week under 50 seeds. Only near-ties differ between seeds, and the solves repeat. Without the memo, the 50 plans take about 12 minutes; with it, about 1–2 s each after the first. `PlanResult.stats.solves` and `cacheHits` therefore depend on what the process has already planned, so the equality checks leave them out.
2. **Improvement alternatives (§5).** The top 5 alternatives are taken from the candidates the day search already solved for that meal, ranked by their PLN-9 score in the whole plan's context, rather than by pre-score from the whole pool. Measured on the F1 week, seeds 1–3: the SC-2 counts moved by at most 1 ingredient either way. Each day costs about 0.4 s less, which the 5 s day budget needs.
3. **Incremental week total (§5).** A swap's effect on the week total is computed exactly, as the swapped meal's new score plus the change in every other meal's economy window, cuisine count and previous-meal terms. It is not recomputed over every pair of meals. `improve.test.ts` checks it against the full recomputation on 40+ swaps. The full recomputation cost about 0.9 s per week and dominated repeat runs.
4. **Re-targeting (§2).** A re-solve may use only the variants the meal already serves, so the PLN-9 §6.4 limit still holds after it. A member's first slot of the day (running deviation 0, band = the slot's own) keeps its beam plate without a re-solve, because the target is identical.
5. **Frequency fallback (R-37, SPEC-Q-15).** When the frequency filter leaves a meal with no dish, the eligible dish served longest ago is used, and the meal is flagged `frequency_relaxed`.
