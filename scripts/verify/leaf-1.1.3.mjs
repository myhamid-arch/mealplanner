...
/** NUT-4 with the generic factors 4/4/9/2 (carbs_g is available carbohydrate, R-20). */
function atwater(i) {
...
/**
 * R-22: NUT-4 with the source's own factors. USDA applies its carbohydrate factor to
 * carbohydrate by difference, so the carbohydrate term is carbs_g + fibre_g and there is no
 * separate fibre term. Null when the entry has no recorded factors.
 */
function atwaterSpecific(i) {
  const f = i.meta?.atwater_factors;
  if (f === null || f === undefined) return null;
  const predicted =
    f.protein * i.protein_g + f.fat * i.fat_g + f.carbohydrate * (i.carbs_g + i.fibre_g);
  if (i.kcal === 0) return predicted === 0 ? 0 : Infinity;
  return (Math.abs(i.kcal - predicted) / i.kcal) * 100;
}

/** R-22: an entry passes NUT-4 when either the generic or its own-factor check is within 12 %. */
function passesNut4(i) {
  const specific = atwaterSpecific(i);
  return (
    atwater(i) <= ATWATER_TOLERANCE_PCT || (specific !== null && specific <= ATWATER_TOLERANCE_PCT)
  );
}
...
function nut4Failures(ings) {
  return ings.filter((i) => !passesNut4(i)).length;
}
...
function checkG2(data) {
  ...
  for (const i of data.snapshot.ingredients) {
    ...
    if (f !== null && f !== undefined)
      p.add(
        [f.protein, f.fat, f.carbohydrate].every((v) => isNum(v) && v >= 0 && v <= 9.5) &&
          [f.protein, f.fat, f.carbohydrate].some((v) => v > 0) &&
          isStr(f.source),
        `${i.slug}: meta.atwater_factors must hold numeric protein, fat and carbohydrate factors and a source`,
      );
    if (passesNut4(i)) {
      // Passing either check never lowers confidence for NUT-4 (R-22).
      p.add(
        !String(i.meta?.confidence_reason ?? "").includes("NUT-4"),
        `${i.slug}: passes NUT-4 but carries a NUT-4 low-confidence reason`,
      );
      continue;
    }
    const specific = atwaterSpecific(i);
    p.add(
      ...
      `${i.slug}: NUT-4 off by ${atwater(i).toFixed(1)} %${specific === null ? "" : ` (${specific.toFixed(1)} % with its own factors)`} and not low confidence with a NUT-4 reason`,
    );
  }
  return p.list;
}
...
function statsG2(data) {
  const ings = data.snapshot.ingredients;
  const fails = ings.filter((i) => !passesNut4(i));
  const genericFails = ings.filter((i) => atwater(i) > ATWATER_TOLERANCE_PCT).length;
  const withFactors = ings.filter((i) => i.meta?.atwater_factors).length;
  const other = structuredClone(data);
  regenerateByDifference(other);
  return [
    `NUT-4 passing: ${String(ings.length - fails.length)} of ${String(ings.length)} (generic 4/4/9/2 alone: ${String(ings.length - genericFails)}; entries with source factors: ${String(withFactors)})`,
...
const GATES = {
  ...
  G2: {
    ...
    controls: [
      [
        "a high-confidence entry with kcal scaled by 1.5",
        (d) => {
          // An entry that passes both checks closely, so the scaled energy cannot pass either.
          const i = d.snapshot.ingredients.find(
            (x) =>
              x.nutrition_confidence === "high" &&
              x.kcal > 50 &&
              atwater(x) < 3 &&
              (atwaterSpecific(x) ?? 0) < 3,
          );
          i.kcal = i.kcal * 1.5;
        },
        "NUT-4 off",
      ...
      [
        ...
        (d) => {
          const i = d.snapshot.ingredients.find((x) => !passesNut4(x));
      ...
      // R-22: an entry that passes only with its own factors must fail without them, or with
      // wrong ones.
      [
        "a specific-factor entry with meta.atwater_factors removed",
        (d) => {
          const i = d.snapshot.ingredients.find(
            (x) => atwater(x) > ATWATER_TOLERANCE_PCT && passesNut4(x),
          );
      ...
      [
        "a specific-factor entry with deliberately wrong factors",
        (d) => {
          const i = d.snapshot.ingredients.find(
            (x) => atwater(x) > ATWATER_TOLERANCE_PCT && passesNut4(x),
          );
          i.meta.atwater_factors = {
            ...i.meta.atwater_factors,
            protein: 9,
            fat: 9,
            carbohydrate: 9,
